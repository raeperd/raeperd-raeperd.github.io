...
CIDR 에서는 네트워크 아이디와 subnet ID는 하나로 통합된다. CIDR는 기존의 address class 의 규칙을 무시한 새로운 규칙이다. 단순히 address class 를 무시했기 때문에, subnet mask로 사용되는 만큼 network ID를 부여할 수있고, 그만큼 호스트 아이디를 가질 수 있다. 규칙이 간단해서 이해하고 적용하기도 쉬운반면 기존의 address class 보다 효율적인 면도 있다.
￼
...
그렇게 라우터는 Data link layer 의 패킷을 받게 되는데, 한번 받은 이후에는 Data-link layer의 정보를 잘라낸다. 
(Ethernet header와 Ethernet footer) IP datagram 에는 10.0.0.10의 주소가 기록되어있는데 라우터는 routing table에서 Network B가 올바른 목적지라는 것을 알 수 있다. 실제로 라우터워 Network B는 직접 연결되어 있기 때문에 router는 10.0.0.10의 MAC 주소까지 ARP 테이블에 가지고 있다.
