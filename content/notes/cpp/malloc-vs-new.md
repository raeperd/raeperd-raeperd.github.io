...
반면, C++ 에서는 동적할당을 `new` 로 한다. 당연히 `new` 는 `malloc` 이후의 문법이기 때문에 이런저런 단점을 극복하려 애를 썼을 것이다. 어떤 차이가 있는지 대충 감으로만 알고 있다가 확실하게 공부를 해봤다.
...
### override
`new` 의 경우 사용자가 정의하는 객체에서 overrides 를 할 수도 있다. `new` 는 operator 이며 `malloc` 은 함수다. (당연히 operator 도 함수의 일부긴 하다.)
...
4. `malloc` 은 재 할당을 할 수 있고, `new` 는 객체에서 override 를 할 수 있다.
...
중요한점은 허용한다고 해서 써도 된다는게 아니라는 점. 문법이 이를 허용하더라도 그냥 사용하지 않는게 좋아보인다. 
...
또 고전적인 C 프로그램의 경우, Heap 영역에서의 취약점의 원인 중 `malloc` 의 type casting 으로 인한 경우가 많이 있다. 기본적으로 `malloc`의 반환값은 `void*` 이기 때문에 이를 변환해줘야 하고 항상 잘못 사용할 가능성을 내포하고 있다는 점에서도 `malloc` 을 쓸 이유가 없다. 적어도 C++ 에서는 그렇다. C에서는 당연하게도 `malloc` 을 사용할 수 밖에 없다.
