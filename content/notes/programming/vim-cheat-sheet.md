...
### find
- `/{word}` 문서에서 단어 찾기 n이나 N으로 다음/이전 찾기
- `*` 현재 단어를 포워드 방향으로 찾기
- `#` 현재 단어를 백워드 방향으로 찾기
...
- `:buffer {number}`  버퍼 목록에 나온 숫자를 입력하면 해당 파일을 오픈함 ( :buffer 대신 `:b {number}` 도 가능)
...
## ETC
- `:colorscheme {schema}` - VIM의 칼라스키마를 변경함(blue, desert, evening 등.. 스키마명에서 탭누르면 자동완성됨)
